// ─────────────────────────────────────────────────────────────────────────────
//  TRUMPALYZER — API Pipeline v8
//  TimesFM response: point_forecast + volatility + stability_score + trend + risk_level
// ─────────────────────────────────────────────────────────────────────────────
import {
  MAX_EVENTS,
  FORECAST_HORIZON,
...
Rules: 2-3 tickers per event, individual signals, NO prices, sort by impact.
...
export async function fetchYahoo(ticker) {
  try {
    ...
    const d = await res.json();
    if (d.error) throw new Error(d.error);
    return { closes: d.closes, current: d.current, change: d.change };
  } catch (err) {
...
export function enrichAllTickersLocally(tickerList) {
  ...
  tickerList.forEach(t => {
    ...
    const rr     = stop !== entry
      ? +(Math.abs(target - entry) / Math.abs(stop - entry)).toFixed(1) : 1.0;
    result[t.ticker] = {
      entry_price: entry, stop_loss: stop, target_24h: target, risk_reward: rr,
...
//  Response: { success, point_forecast, volatility, stability_score, trend, risk_level }
export async function fetchTimesFMWithConvergence(closes, claudeSignal) {
  ...
  let values    = null;
  let simulated = false;
  let meta      = {};  // volatility, stability_score, trend, risk_level

  try {
    const res = await fetch("/api/timesfm", {
      method:  "POST",
      headers: { "Content-Type": "application/json" },
      body:    JSON.stringify({ inputs: [input], horizon: FORECAST_HORIZON }),
    });

    if (!res.ok) throw new Error(`TimesFM proxy ${res.status}`);
    const data = await res.json();

    if (!data.success && !data.point_forecast) throw new Error("No forecast in response");

    // ← Correct response key from README
    values = data.point_forecast?.slice(0, FORECAST_HORIZON).map(v => +v.toFixed(2));
    if (!values?.length) throw new Error("Empty point_forecast");
    ...
    console.log(`[TimesFM] Real forecast OK — trend:${meta.trend} risk:${meta.riskLevel}`);
